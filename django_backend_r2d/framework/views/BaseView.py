    ...
    def handle_exceptions(func):
        ...
        def wrapper(view_instance, request, *args, **kwargs):
            ...
            # DiagramConsumerExceptions
                ...
                logger.error(repr(e), exc_info=True)
                return JSONResponse(data = {"error": str(e)}, message = "Error occurred in Class Diagram Task", success = False, status_code=status.HTTP_400_BAD_REQUEST).transform()
            except ClassDiagramConsumerError as e:
                logger.error(repr(e), exc_info=True)
                return JSONResponse(data = {"error": str(e)}, message = "Error occurred in Class Diagram Consumer", success = False, status_code=status.HTTP_400_BAD_REQUEST).transform()
            except ClassDiagramSignalError as e:
                logger.error(repr(e), exc_info=True)
                return JSONResponse(data = {"error": str(e)}, message = "Error occurred in Class Diagram Signal", success = False, status_code=status.HTTP_400_BAD_REQUEST).transform()
            except ERDiagramTaskError as e:
                logger.error(repr(e), exc_info=True)
                return JSONResponse(data = {"error": str(e)}, message = "Error occurred in ER Diagram Task", success = False, status_code=status.HTTP_400_BAD_REQUEST).transform()
            except ERDiagramConsumerError as e:
                logger.error(repr(e), exc_info=True)
                return JSONResponse(data = {"error": str(e)}, message = "Error occurred in ER Diagram Consumer", success = False, status_code=status.HTTP_400_BAD_REQUEST).transform()
            except ERDiagramSignalError as e:
                logger.error(repr(e), exc_info=True)
                return JSONResponse(data = {"error": str(e)}, message = "Error occurred in ER Diagram Signal", success = False, status_code=status.HTTP_400_BAD_REQUEST).transform()
            except DiagramCreationSignalError as e:
                logger.error(repr(e), exc_info=True)
                return JSONResponse(data = {"error": str(e)}, message = "Error occurred in Signal", success = False, status_code=status.HTTP_400_BAD_REQUEST).transform()
