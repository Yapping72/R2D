...
class ClassDiagramAuditAnalyzeChainInputs(BaseChainInput): 
    ...
    """ 
    def __init__(self, model_prompt_template=ClassDiagramPromptTemplate(), audit_prompt_template=AuditClassDiagramPromptTemplate(), analysis_context=None, audit_criteria=None, model_response_schema=MERMAID_CLASS_DIAGRAM_SCHEMA, auditor_response_schema=MERMAID_CLASS_DIAGRAM_SCHEMA):
        """
        Initialize the chain inputs for the class diagram chain.
        """
        super().__init__(model_prompt_template, audit_prompt_template, analysis_context, audit_criteria, model_response_schema, auditor_response_schema)